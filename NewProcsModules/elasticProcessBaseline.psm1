function Get-ElasticProcessBaseline {
    ...
    # Required regardless: vault secret Elastic_URL (e.g.
    # 'https://192.168.71.10/elasticsearch' for SO, or
    # 'https://elasticsearch.lab:9200' for vanilla Elastic).
    ...
    if ($esUrl -match '^http://') {
        ...
        try {
            [void](Invoke-RestMethod -Uri "$httpsUrl/_cluster/health" -Headers $esHeaders -Method Get @restArgs -ErrorAction Stop)
    ...
    # to its SOC login (HTTP 302 to /login). Warn the user if Elastic_URL points
    # at the SO root instead of the /elasticsearch proxy path.
    try {
        Invoke-WebRequest -Uri "$esUrl/_cluster/health" -Headers $esHeaders -Method Get -MaximumRedirection 0 @restArgs -ErrorAction Stop | Out-Null
    } catch {
    ...
    function Invoke-ESQuery {
        ...
        try {
            return Invoke-RestMethod -Uri $uri -Headers $esHeaders -Method Post -Body $bodyJson @restArgs
        } catch {
            $code = $null
